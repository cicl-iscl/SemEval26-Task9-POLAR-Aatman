...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Finetune XLM-RoBERTa-large on Subtask 1"
    )
    parser.add_argument(
        "--model_name",
        type=str,
    ...
    parser.add_argument(
        "--data_dir",
        type=str,
        default="../../../data/subtask1",
        help="Path to data directory",
    )
    parser.add_argument(
        "--output_dir",
        type=str,
        default="./finetune_results_xlm_roberta",
        help="Output directory for results",
    )
    parser.add_argument(
        "--submission_dir",
        type=str,
        default="./subtask_1_xlm_roberta",
...
def main():
    ...
    # 6. Training Setup
    steps_per_epoch = len(encoded_dataset["train"]) // (
    ...
    submission_dataset = Dataset.from_pandas(
        raw_test_df[["text"]], preserve_index=False
    )
    submission_tokenized = submission_dataset.map(tokenize_function, batched=True)
    submission_tokenized.set_format(
